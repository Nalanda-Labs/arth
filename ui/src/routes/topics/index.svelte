...
<script>
  ...
  async function fetchData() {
    ...
    let response = await api.post(`topics/`, { updated_at: updated_at });
    ...
    for (let i = 0; i < topics.length; i++) {
      ...
      if (asked_ts == updated_ts) {
        ...
        if (shown_ts >= 259200) {
          ...
          shown_ts =
            "asked on " + asked_ts.getDate() + "/" + month + "/" + year;
        ...
        if (shown_ts >= 259200) {
          ...
          shown_ts =
            "modified on " + updated_ts.getDay() + "/" + month + "/" + year;
        } else if (172800 <= shown_ts && shown_ts < 259200) {
      ...
      <div style="width:85%;float:left;position:relative">
        ...
        <span style="float:right"
          ...
          <a
            ...
            style="text-decoration:none; color:#4285F4;">{username}</a
          ></span
        >
      </div>
    {/each}
